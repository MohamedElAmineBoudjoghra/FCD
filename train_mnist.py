...
def parse_args():
    ...
    parser.add_argument("--byz_type", help="type of attack", default='MPAF', type=str,
...
def main(args):
    ...
    with ctx:
        ...
        for _, (data, label) in tqdm(enumerate(train_data),desc="Train dataloader"):
        ...
        for e in range(epochs):
            ...
            for i in tqdm(range(100), desc = f"Epoch: {e}"):
                ...
                weight_list.append([param.data() for param in net.collect_params().values()])

            param_list = [nd.concat(*[xx.reshape((-1, 1)) for xx in x], dim=0) for x in grad_list]
            weight_list_ = [nd.concat(*[xx.reshape((-1, 1)) for xx in x], dim=0) for x in weight_list]
            tmp = []
            for param in net.collect_params().values():
                tmp.append(param.data().copy())
            ...
            # perform attack
            if e%100 >= 50:
                if args.byz_type != 'MPAF':
                    param_list = byz(param_list, args.nbyz)
                else:
                    param_list = byz(weight_list_, args.nbyz)
